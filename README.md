...
sudo dd if=bishos-x86_64_v0.6.1.iso of=/dev/sdX bs=4M conv=fsync
...
4. **Build a bootable ISO** -- GRUB on both architectures, same layout and
   ...
   ```bash
   make ARCH=arm64 iso   # -> build/arm64/bishos-arm64_v0.6.1.iso   (UEFI)
   make iso              # -> build/x86_64/bishos-x86_64_v0.6.1.iso (UEFI + BIOS)
   ```
...
5. **Install software** -- the persistent root ships with `apk`:
   ...
   ```bash
   oras pull ghcr.io/bishalramdam/bishos:0.6.1-arm64
   oras pull ghcr.io/bishalramdam/bishos:0.6.1-x86_64
   ```
